...
CC := gcc
CFLAGS := $(shell pkg-config --cflags lv2) -DAMP_URI=\"$(PLUGIN_URI)\"
...
build/lv2_test.lv2/lv2_test.so: build/main.o | build/lv2_test.lv2/
	...
	sed -e "s|@LIB_EXT@|.so|g" -e "s|@APP_URI@|$(PLUGIN_URI)|g" manifest.ttl > build/lv2_test.lv2/manifest.ttl 
	sed -e "s|@LIB_EXT@|.so|g" -e "s|@APP_URI@|$(PLUGIN_URI)|g" lv2_test.ttl > build/lv2_test.lv2/lv2_test.ttl 
	$(CC) $(LDFLAGS) -o $@ $?
